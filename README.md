...
| <a name="input_dedicated_master_key_keysharefile"></a> [dedicated\_master\_key\_keysharefile](#input\_dedicated\_master\_key\_keysharefile) | List of key-share file configurations used to initialize the dedicated Key Protect instance. Each entry represents one key-share part (minimum 2 required). Only used when `plan` is `dedicated`. | <pre>list(object({<br/>    filepath   = string<br/>    passphrase = string<br/>  }))</pre> | <pre>[<br/>  {<br/>    "filepath": "kp-dedicated-mbk-1.key",<br/>    "passphrase": ""<br/>  },<br/>  {<br/>    "filepath": "kp-dedicated-mbk-2.key",<br/>    "passphrase": ""<br/>  }<br/>]</pre> | no |
