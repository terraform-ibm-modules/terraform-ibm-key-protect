...
| <a name="input_dedicated_master_key_keysharefile"></a> [dedicated\_master\_key\_keysharefile](#input\_dedicated\_master\_key\_keysharefile) | JSON-encoded map of key-share file configurations used to initialize the dedicated Key Protect instance. Each key is a string index and each value must have `filepath` and `passphrase` fields. Minimum 2 entries required. Example: `{"0":{"filepath":"mbk-1.key","passphrase":""},"1":{"filepath":"mbk-2.key","passphrase":""}}`. Only used when `plan` is `dedicated`. | `string` | `"{\"0\":{\"filepath\":\"kp-dedicated-mbk-1.key\",\"passphrase\":\"\"},\"1\":{\"filepath\":\"kp-dedicated-mbk-2.key\",\"passphrase\":\"\"}}"` | no |
