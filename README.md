...
| <a name="input_dedicated_master_key_keysharefile"></a> [dedicated\_master\_key\_keysharefile](#input\_dedicated\_master\_key\_keysharefile) | JSON-encoded list of key-share file configurations used to initialize the dedicated Key Protect instance. Each entry must have `filepath` and `passphrase` fields. Minimum 2 entries required. Example: `[{"filepath":"mbk-1.key","passphrase":""},{"filepath":"mbk-2.key","passphrase":""}]`. Only used when `plan` is `dedicated`. | `string` | `"[{\"filepath\":\"kp-dedicated-mbk-1.key\",\"passphrase\":\"\"},{\"filepath\":\"kp-dedicated-mbk-2.key\",\"passphrase\":\"\"}]"` | no |
